...
### 姿をどう撮るか（2026-07-31 追記）

iOS はシミュレータで足りる。`xcrun simctl io booted screenshot` は画面の状態に依らない。

macOS で詰まった。`screencapture` は画面収録の許可が要り、画面が消灯していると真っ黒を返し、
窓を掴みそこねると何も撮れない。実際そこで何度も止まった。

窓を画面の外へ置いて `cacheDisplay` でビットマップへ落とせば、そのどれも要らない。
`sheet` は別の窓として出るので `attachedSheet` から辿る。道具は `Tools/shoot-macos.swift`。
...
最後の一行が残る穴である。「背面を押しても閉じない」のような、触った後にどうなるかは
この手では届かない。`XCUITest` を組むか、人に押してもらうことになる。
